...
{
  if (p == 256) return 0;
  if (k == 0) return 1e18; //1e18 is 1x10^18
...
int main() 
{
  ...
  while (cin >> D >> K) 
  {
    memset(nv, 0, sizeof(nv)); // set every element in nv to be 0
    for (int i = 0; i < D; i++) 
    {
      cin >> R >> P;
      nv[R] += P;
    ...
    memset(memo, -1, sizeof(memo)); // set every element in memo to be -1
